...
class _NamespacedSessionMap(MutableMapping[str, str]):
    ...
    Every key the caller uses (the bare conv_key) is stored under
    ``"<engine_type>:<key>"`` in the underlying map, so an opencode ``ses_…`` id and
    a Pi session-file path can never collide on a persisted home whose
    ``engine.type`` flipped between runs. The pool wraps its store in this so
    ``run_turn``/``engine_runner`` keep using bare keys.
    """
...
class EnginePool:
    """Pool of engine servers keyed by session_key.
    ...
    def __init__(
        ...
        *,
        ...
        # In production it points to the selected driver's launch method. Keep
        # construction tolerant of lightweight driver doubles used for map-only tests.
        launch = getattr(self._driver, "launch", None)
        if not callable(launch):

            async def _missing_launch(config: EngineConfig, session_key: str) -> ManagedServer:
                raise RuntimeError("Engine driver does not implement launch")
