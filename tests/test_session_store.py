...
"""Unit tests for boot.stores.open_session_store — persistent vs in-memory selection."""
...
def test_open_session_store_disabled_is_in_memory() -> None:
    from ach_agent.boot.stores import open_session_store
    from ach_agent.engine.pool import _LRUSessionMap, _SqliteSessionMap

    store = open_session_store(_cfg({"enabled": False}))
...
def test_open_session_store_enabled_persists_to_state_db(tmp_path: Path) -> None:
    ...
    store = open_session_store(_cfg({"enabled": True, "mountPath": str(tmp_path)}))
