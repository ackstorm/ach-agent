...
def test_provider_api_mapping_and_no_ek(monkeypatch) -> None:
    ...
    assert doc[provider]["apiKey"] == "local-proxy"
    assert doc[provider]["headers"] == {}
...
def test_openai_and_anthropic_api_kinds() -> None:
    ...
    assert doc_o[provider_o]["api"] == "openai-completions"
    assert doc_a[provider_a]["api"] == "anthropic-messages"
