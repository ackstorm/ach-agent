...
| 2026-07-23 | [pi-model-capability-and-thinking](2026-07-23-pi-model-capability-and-thinking.md) | Superseded | Pi's `reasoning`/`input`/`contextWindow`/`maxTokens`/`--thinking` get a typed, generated `engine.pi.model`/`engine.pi.thinkingLevel` surface (not `model.params`, which stays opaque per-call passthrough); `model.type` wire selection untouched. |
| 2026-07-24 | [model-owned-thinking](2026-07-24-model-owned-thinking.md) | Shipped | Thinking intent moves to a normalized `model.thinking`; `engine.pi` back to executable knobs only; each engine translates. |
| 2026-08-31 | [channel-owned-repo-workspace](2026-08-31-channel-owned-repo-workspace.md) | Shipped (v0.13.0) | Harness prepares the repo per event: credentials + origin on the channel, executed on the lane (post-dedup), workspace as the engine cwd. Landed as the generic `channel.prepare` script hook, not a typed clone block. |
| 2026-09-02 | [webhook-script-channel](2026-09-02-webhook-script-channel.md) | Shipped (v0.13.4) | `webhook-script`: authenticated GitLab ingress that runs a static `/bin/sh` handler on a project-scoped lane and never starts an engine. Requires `source: gitlab` + explicit `gitlabEvents`; project/system kinds are exclusive to it. |
| 2026-09-09 | [ach-memory-backend](2026-09-09-ach-memory-backend.md) | Spec (settled) | `ach-memory` replaces direct-Hindsight as the memory backend: per-repo scope resolved server-side through authz instead of a tag convention, budgeted `load_context` instead of four unbounded mental-model injections, `POST /v1/bootstrap` for provisioning. **Supersedes [hindsight-memory-facade](2026-07-05-hindsight-memory-facade.md) for new agents.** |
