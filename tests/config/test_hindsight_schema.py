...
def test_rich_mental_models_parse_with_aliases():
    ...
    assert (mm.id, mm.name, mm.source_query) == (
        "architecture",
        "Arch",
...
def test_bank_must_be_static():
    # T-04-03: bank is harness-owned + static — templating is forbidden (payload is untrusted,
    # and the facade uses the static boot bank; a {{ }} bank would diverge from the fetch).
    with pytest.raises(ValidationError):
        HindsightParams.model_validate(_base(bank="pr-{{ payload.project }}"))


def test_static_bank_accepted():
    assert HindsightParams.model_validate(_base(bank="gitlab-ackstorm")).bank == "gitlab-ackstorm"
