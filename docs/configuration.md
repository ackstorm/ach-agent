...
| `memory` | | Fail-open — backend down → run without it. Discriminated on `type`. `codemem`: local stdio MCP, `dbPath` / `project` both derived by default. `ach-memory`: exactly one of `endpoint` (the **complete** MCP endpoint, used verbatim — the harness appends nothing) or `mcpServerId` (the hydrated `runtime.mcpServers[].id`, whose endpoint is read from the manifest **and which is then excluded from the MCP proxy** — without that the agent reaches the same service twice, once through the facade that pins scope and `project_slug`, and once directly with the ek_ where `project_slug` is just an argument; the exclusion comes from config alone and is applied before auth resolves, so a degrade leaves nothing reachable rather than the raw surface), `auth` (`{type: ach}` → the harness's own ek_ as `x-ach-key`, for reaching ach-memory through ACH's gateway; `{type: bearer, env: …, header: …}` → a token sent direct, not a bank-wide admin secret. ach-memory mints no credentials, so what that token must *be* follows from the header it rides: `header: Authorization` (the default) reaches the **JWT** provider and needs a JWT its configured issuer signed; `header: x-litellm-api-key` — or whatever that deployment sets as `MEMORY_AUTH_PLATFORM_INCOMING_HEADER` — reaches the **platform** provider and needs a key its resolver can name. `Bearer ` is prepended only on `Authorization`; every other header carries the raw secret. Omitting `auth` sends no credential, which no current ach-memory accepts), optional `project`. Standing context is assembled and budgeted server-side; the harness injects scope and project below the agent, so neither is configurable and neither appears on any exposed tool. |
...
| `webhook-script` | `webhook` (+ `gitlabEvents`) + `script` + `source: gitlab` | Authenticated asynchronous webhook that runs a static `/bin/sh` script and never invokes the model. |
...
stdin **with a trailing newline** (so `read -r line` and `while read` both work), validated
event fields are available as `ACH_EVENT_*`, and `ACH_WORKSPACE` is temporary. The script runs
asynchronously after the endpoint returns `202`; every run is counted by
`ach_agent_webhook_script_runs_total{channel,status}` and failures also appear in logs and
`ach_agent_webhook_script_failures_total`.

It requires `source: gitlab` and an explicit `webhook.gitlabEvents` — there is no default,
...
that lists one is rejected at load. Project events use one lane per project and channel,
preventing concurrent reconciliation of the same project.
...
memory:
  ...
  achMemory:
    ...
    # Prefer naming the hydrated server over writing its URL: the endpoint then comes from
    # the manifest, and the harness drops that server from the MCP proxy so the agent
    # reaches memory ONLY through the facade:
    #   mcpServerId: ach-memory
    #   auth: {type: ach}
    # Exactly one of `endpoint` or `mcpServerId`.
